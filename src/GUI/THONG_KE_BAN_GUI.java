...
public class THONG_KE_BAN_GUI extends JPanel {
  ...
  THONG_KE_BAN_BUS thong_KE_BAN_BUS = new THONG_KE_BAN_BUS();

  private JTable jTable_SanPhamBan;
  private NonEditableTableModel tableModel_SanPhamBan;
  private JScrollPane jScrollPane_SanPhamBan;
  private DefaultCategoryDataset  dataset;
  private JFreeChart chart;

  public THONG_KE_BAN_GUI() {
  ...
  public void init_ThongKeNhap() {
    ...
    jTable_SanPhamBan = new JTable();
    tableModel_SanPhamBan = new NonEditableTableModel(new Object[][] {
    }, new String[] { "STT", "MÃ XE", "TÊN XE", "GIÁ NHẬP", "SỐ LƯỢNG", "TỔNG" });
    jTable_SanPhamBan = new JTable(tableModel_SanPhamBan);
    jScrollPane_SanPhamBan = new JScrollPane(jTable_SanPhamBan);
    jScrollPane_SanPhamBan.setBounds(20, 140, 640, 380);

    dataset = new DefaultCategoryDataset();

    // Tạo biểu đồ từ dataset
    chart = ChartFactory.createBarChart(
        "THỐNG KÊ SỐ LƯỢNG BÁN XE",
        "XE MÁY", "SỐ LƯỢNG BÁN", dataset);
    ...
    this.add(jScrollPane_SanPhamBan);
    this.add(chartPanel);

  }

  public void thongKeNhapXe(java.awt.event.ActionEvent evt) {
    tableModel_SanPhamBan.setRowCount(0);
    ...
    if (jTextField_Nam.getText().equals("")) {
      ...
      arrCTHDB = thong_KE_BAN_BUS.getAllCTHDB_ThongKe(ngayLap);
      if (arrCTHDB.size() == 0) {
        JOptionPane.showMessageDialog(this, "Không có xe nào được bán trong thời gian này", "ERROR",
            JOptionPane.ERROR_MESSAGE);
        tableModel_SanPhamBan.setRowCount(0);
      } else {
        DecimalFormat decimalFormat = new DecimalFormat("###,###");
        for (int i = 0; i < arrCTHDB.size(); i++) {
          CT_HOA_DON_BAN_DTO ct_HOA_DON_NHAP_DTO = arrCTHDB.get(i);
          Object[] newrow = { i + 1, ct_HOA_DON_NHAP_DTO.getMA_XE(), ct_HOA_DON_NHAP_DTO.getTEN_XE(),
              decimalFormat.format(ct_HOA_DON_NHAP_DTO.getGIA_NHAP()), ct_HOA_DON_NHAP_DTO.getSO_LUONG(),
              decimalFormat.format(ct_HOA_DON_NHAP_DTO.getGIA()) };
          tableModel_SanPhamBan.addRow(newrow);
  ...
  public void veBieuDo(ArrayList<CT_HOA_DON_BAN_DTO> arrCTHDB) {
